...
def train_cycle_gan(load_models, batch_size, loss, num_epochs, label_smoothing, save_models, learning_rate):
    ...
    if loss == "BCE":
        discriminator_young.sigmoid = True
        discriminator_old.sigmoid = True

    if load_models is not None:
        discriminator_young.load_state_dict(torch.load(f"{load_models}/d_young.pth"))
        discriminator_old.load_state_dict(torch.load(f"{load_models}/d_old.pth"))
        generator_young_to_old.load_state_dict(torch.load(f"{load_models}/g_yto.pth"))
        generator_old_to_young.load_state_dict(torch.load(f"{load_models}/g_oty.pth"))

    discriminator_young_optimizer = torch.optim.Adam(discriminator_young.parameters(), lr=learning_rate, betas=(0.5, 0.999))
    discriminator_old_optimizer = torch.optim.Adam(discriminator_old.parameters(), lr=learning_rate, betas=(0.5, 0.999))
    generator_yto_optimizer = torch.optim.Adam(generator_young_to_old.parameters(), lr=learning_rate, betas=(0.5, 0.999))
    generator_oty_optimizer = torch.optim.Adam(generator_old_to_young.parameters(), lr=learning_rate, betas=(0.5, 0.999))

    adversarial_loss = nn.MSELoss() if loss == "MSE" else nn.BCELoss() # Change this to BCE Loss for normal
    ...
    for epoch in range(num_epochs):
        ...
        for young_images, old_images in zip(young_faces_dataloader, old_faces_dataloader):
            ...
            if iteration % 200 == 0:
                zanette_image = Image.open("test_images/zanette.png").convert("RGB")
                savvides_image = Image.open("test_images/savvides.png").convert("RGB")
                ...
                if save_models is not None:
                    torch.save(discriminator_old.state_dict(), f"{save_models}/d_old.pth")
                    torch.save(discriminator_young.state_dict(), f"{save_models}/d_young.pth")
                    torch.save(generator_old_to_young.state_dict(), f"{save_models}/g_oty.pth")
                    torch.save(generator_young_to_old.state_dict(), f"{save_models}/g_yto.pth")

            iteration += 1

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train a Cycle GAN for Facial Reaging")
    parser.add_argument("--load_models", type=str, default=None, help="Directory of model weights to load from")
    parser.add_argument("--save_models", type=str, default=None, help="Directory to save model weights")
    parser.add_argument("--batch_size", type=int, default=16, help="Batch size to use for training")
    parser.add_argument("--num_epochs", type=int, default=50, help="Number of epochs to train the Cycle GAN")
    parser.add_argument("--learning_rate", type=float, default=2e-4, help="Learning rate for all the models")
    parser.add_argument("--loss", type=str, default="BCE", help="Loss to train the Cycle GAN. Type BCE or MSE.")
    parser.add_argument("--label_smoothing", action="store_true", help="Whether to use Label Smoothing for the loss")

    args = parser.parse_args()
    train_cycle_gan(args.load_models, args.batch_size, args.loss, args.num_epochs, args.label_smoothing, args.save_models, args.learning_rate)
